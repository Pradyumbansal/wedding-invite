...
export default function Wardrobe() {
  return (
    <section id="wardrobe" data-testid="wardrobe-section" className="relative px-4 py-20 sm:py-28">
        ...
        <div className="grid gap-8 sm:grid-cols-3 sm:gap-6 lg:gap-8">
          {INVITATION.wardrobe.map((w, i) => (
            <Reveal key={w.id} delay={i * 0.12}>
              <div
                className="gold-frame flex h-full flex-col bg-[#FDFBF7] p-4"
                data-testid={`wardrobe-card-${w.id}`}
              >
                <div className="overflow-hidden border border-[#D4AF37]/35">
                  <img
                    src={w.image}
                    alt={`${w.event} wardrobe inspiration`}
                    loading="lazy"
                    className="aspect-square w-full object-cover"
                  />
                </div>
                <p className="font-label mt-6 text-center text-[9px] text-[#D4AF37]">
                  {w.date}
                </p>
                ...
                <p className="font-label mt-auto pb-2 pt-4 text-center text-[10px] leading-relaxed text-[#B8860B]">
                  | {w.dressCode} |
                </p>
              </div>
            </Reveal>
